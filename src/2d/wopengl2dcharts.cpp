...
HACCEL Opengl2DChart::m_hacelerator;
int COGLGraph::m_id_dialogos=5000;//!< Identificador del siguiente dialogo ES ACCESIBLE DESDE TODAS LAS INSTANCIAS
CWnd *Opengl2DChart::m_Pdialogs[50];
HWND COGLGraph::m_hwndvisibilidad=NULL;		//!< Ventana de visibilidad.

Opengl2DChart::Opengl2DChart( void*pParent)
{
 InicializacionComun();
 m_pparent=(CWnd*)pParent;
 if(Opengl2DChart::m_ndialogs==0)// inicializacion de los miembros static
 {
	 memset(Opengl2DChart::m_pgraficas,0,sizeof(Opengl2DChart::m_pgraficas));//inicializamos el vector de graficas
 ...
 Opengl2DChart::m_ndialogs++;
 COGLGraph::m_ngraficas++;
 Opengl2DChart::m_Pdialogs[COGLGraph::m_ndialogs-1]=this;
}
void Opengl2DChart::CreaTooltipW()
{
	DWORD flagext,flags;
	TOOLINFO ti={0};
...
Opengl2DChart::~Opengl2DChart()
{
	if (m_hacelerator!=NULL&&Opengl2DChart::m_ngraficas==0)
	{
...
BOOL Opengl2DChart::CreateAndPosition(   void* pParentWnd,CRect pos,int popup)
{
	BOOL creado;
...
void Opengl2DChart::MenuContextualW(CPoint point)
{
	ContextMenu();
}
int Opengl2DChart::AceleratorsW(MSG* pMsg)
{
...
void Opengl2DChart::RePaint()
{
	if(this->IsIconic()==0)
		Draw();
}
POINT Opengl2DChart::GetMousePoint()
{
	POINT pt;
	CPoint ptw;
	GetCursorPos(&ptw);
...
void Opengl2DChart::Dialogodouble(int popup)
{
	if(popup)
	{
...
void Opengl2DChart::CambiaBordeDlg()
{
...
void Opengl2DChart::Mostrar()
{

}
BEGIN_MESSAGE_MAP(Opengl2DChart, CWnd)
ON_WM_PAINT()
ON_WM_CLOSE()
ON_WM_SIZING()
ON_WM_RBUTTONDOWN()
...
ON_WM_DROPFILES()
END_MESSAGE_MAP()
void Opengl2DChart::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void Opengl2DChart::OnClose()
{
	OnCloseEvent();
}
void Opengl2DChart::OnSizing(UINT fwSide, LPRECT pRect)
{
	CRect rct;
	GetClientRect(rct);
	m_ancho=rct.Width();
...
LRESULT CALLBACK Opengl2DChart::WindowProc(HWND hwnd,UINT Mensaje,WPARAM wparam,LPARAM lparam)
{
	return (::DefWindowProc(hwnd,Mensaje,wparam,lparam));
}
BOOL Opengl2DChart::PreTranslateMessage(MSG* pMsg)
{
	if(Acelerators(pMsg))
		return TRUE;
	return CWnd::PreTranslateMessage(pMsg);
}
afx_msg void Opengl2DChart::OnLeftKey()
{
	 OnKeyPress(k_left);
}
afx_msg void Opengl2DChart::OnRightKey()
{
	 OnKeyPress(k_right);
}
afx_msg void Opengl2DChart::OnUpKey()
{
	 OnKeyPress(k_up);
}
afx_msg void Opengl2DChart::OnDownKey()
{
	 OnKeyPress(k_down);
}
void Opengl2DChart::OnLButtonDblClk(UINT nFlags, CPoint point)//ejes auxiliares
{
	POINT point(point.x(),point->pos().y());
	OnDoubleClic(point,flags);
	CWnd::OnLButtonDblClk(nFlags, point);
}
void Opengl2DChart::OnLButtonDown(UINT nFlags, CPoint point)
{
	POINT point(point.x(),point->pos().y());
	OnLMousePress(point,nFlags);
	CWnd::OnLButtonDown(nFlags, point);
}
void Opengl2DChart::OnLButtonUp(UINT nFlags, CPoint point)
{
	POINT point(point.x(),point->pos().y());
...
void Opengl2DChart::OnRButtonDown(UINT nFlags, CPoint point)
{
	POINT point(point.x(),point->pos().y());
	OnRMousePress(point,nFlags);
	CWnd::OnRButtonDown(nFlags, point);
}
void Opengl2DChart::OnMButtonDown(UINT nFlags, CPoint point)
{
	POINT point(point.x(),point->pos().y());
	OnRMousePress(point,nFlags);
	CWnd::OnMButtonDown(nFlags, point);
}
void Opengl2DChart::OnMButtonUp(UINT nFlags, CPoint point)
{
	POINT point(point.x(),point->pos().y());
	OnMMouseRelease(point,nFlags);
	CWnd::OnMButtonUp(nFlags, point);
}
BOOL Opengl2DChart::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	POINT point(point.x(),point->pos().y())
	OnMouseWheel(point,nFlags,zDelta);
}
void Opengl2DChart::OnSize(UINT nType, int cx, int cy)
{
	m_ancho=cx;
	m_alto=cy;
	OnResize();
}
void Opengl2DChart::OnMouseMove(UINT nFlags, CPoint point)
{
	POINT point(point.x(),point->pos().y());
...
bool COGLGraph::SaveAs()
{
	...
	if(archivo.DoModal()==IDCANCEL)
			return 0;
	CString direccion=archivo.GetPathName();
